...
/** Five distinct, gently evolving sound worlds rendered locally for private, offline reminders. */
final class ProceduralSoundscape {
    ...
    void play(int preset, int seconds) {
        stop();
        running = true;
        renderThread = new Thread(() -> render(Math.max(0, Math.min(4, preset)), Math.max(8, seconds)),
                "prayer-soundscape");
        renderThread.start();
    }
    ...
    private void render(int preset, int seconds) {
        ...
        double strikeAt = preset == 2 ? 0.8 : 1.7;
        ...
        for (int base = 0; running && base < totalFrames; base += frames) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                double breath = .5 - .5 * Math.cos(Math.PI * 2 * time / (preset == 4 ? 7.5 : 11.0));
                double pad = 0;
                ...
                for (int voice = 0; voice < 4; voice++) {
                    ...
                    if (preset == 1 || preset == 3 || preset == 4) {
                        pad += Math.sin(phases[voice] * .5) * weight * .14;
                    }
                }

                double gap = preset == 0 ? 5.5 : preset == 1 ? 9.0 : preset == 2 ? 2.25 : preset == 3 ? 7.0 : 4.2;
                ...
                if (age >= 0 && age < 5.8) {
                    ...
                    double attack = Math.min(1, age / (preset == 2 ? .018 : .09));
                    double decay = Math.exp(-age * (preset == 2 ? 1.35 : .67));
                ...
                double padLevel = preset == 0 ? .026 : preset == 1 ? .070 : preset == 2 ? .020 : .050;
                double toneLevel = preset == 0 ? .12 : preset == 1 ? .025 : preset == 2 ? .105 : preset == 3 ? .030 : .045;
                double pulse = preset == 4 ? .82 + breath * .18 : 1.0;
